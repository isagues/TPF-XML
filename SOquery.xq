...
{
    (for $p in doc("Posts.xml")/posts/row[$Id = @OwnerUserId and @Score = max(//row[$Id = @OwnerUserId]/@Score)]
...
<result xmlns:xsi = "http://www.w3.org/2001/XMLSchema-instance" xsi:noNamespaceSchemaLocation="./intermediate.xsd">{
    if (count(doc("Users.xml")/users/row[@Reputation > $threshold]) = 0)
                ...
                for $u in doc("Users.xml")/users/row[@Reputation > $threshold]
                    ...
                    return
                        <user Id="{$u/@Id/string()}">
                            ...
                            {if(count(doc("Posts.xml")/posts/row[@OwnerUserId = $u/@Id]) = 0)
                            then ()
